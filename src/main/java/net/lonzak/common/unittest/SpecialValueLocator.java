...
/**
 * Sometimes while auto testing classes it is necessary to provide special formats or ranges [...]. This can be done
 * using the special value mechanism provides by this class.
 * 
 * @author Lonzak
 *
 */
public final class SpecialValueLocator {
  ...
  /**
   * Initializes the list of special values with one value. To add further values use the
   * {@link #addSpecialValue(ConstructorValue)} method.
   * 
   ...
   * 
   * This variable is set while iterating over the constructors in the AutoTester. Not to be publicly called.
   * 
  ...
  /**
   * Retrieves the special value by using the parameterIndex. The umberOfArguments must be set in advance.
   * {@link SpecialValueLocator#setNumberOfArgumentsConstructor(int)}
   * 
   * @param parameterIndex of the special value to retrieve
   * @return the special object or null
   */
  Object getSpecialValue(int parameterIndex, Class<?> dataType) {
    if (this.numberOfArgumentsConstructor != -1) {
      ConstructorValue specialValue = this.specialValues
          .get(new ConstructorValue(this.numberOfArgumentsConstructor, parameterIndex, null, dataType, true));
  ...
  Class<?> getDataType(int parameterIndex, Class<?> dataType) {
    if (this.numberOfArgumentsConstructor != -1) {
      ConstructorValue value = this.specialValues
          .get(new ConstructorValue(this.numberOfArgumentsConstructor, parameterIndex, null, dataType, true));
  ...
  public String toString() {
    return "SpecialValueLocator [specialValues=" + this.specialValues + ", numberOfArguments="
        + this.numberOfArgumentsConstructor + "]";
  }

  /**
   * The location element of a certain value which should be set. A location is defined by the number of arguments and
  ...
  public static final class ConstructorValue {
    ...
    /**
     * The location identifies which parameter of which constructor to set. The first parameter (numberOfArguments) is
     * to identify the constructor. A constructor can be identified by the number of arguments it has.
     * <p>
     * - The argument is defined by its index e.g. the 3rd parameter should be set so its index is 3.
     * <p>
     * - The index is '1' based. Meaning that the first attribute of a 1 argument constructor is 1,1.
     * <p>
    ...
    ConstructorValue(int numberOfArguments, int parameterIndex, Object value, Class<?> dataType, boolean internalCall) {
      if (!internalCall && numberOfArguments != 0 && numberOfArguments < parameterIndex) {
        throw new IllegalArgumentException("The number of arguments (" + numberOfArguments
            + ") can not be smaller than the parameter index (" + parameterIndex + ")!");
      }
      if (numberOfArguments < 0 || parameterIndex < 1) {
        throw new IllegalArgumentException("Illegal value for number of constructor arguments (" + numberOfArguments
            + ") or the parameter index(" + parameterIndex + ")!");
      }
    ...
    public boolean compareCoordinates(ConstructorValue constructorValue) {
      return this.numberOfArguments == constructorValue.getNumberOfArguments()
          && this.parameterIndex == constructorValue.getParameterIndex();
    }
    ...
    public String toString() {
      return "Location [numberOfArguments=" + this.numberOfArguments + ", parameterIndex=" + this.parameterIndex
          + ", value: " + this.value + ", data type: " + this.dataType + "]";
    }
  }
}
